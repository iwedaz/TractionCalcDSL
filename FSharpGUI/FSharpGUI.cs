...
namespace FSharpGUI
{
	public partial class FSharpGUI : Form
	{
		...
		public FSharpGUI()
		{
			...
			//init menu images
			ComponentResourceManager resources = new ComponentResourceManager(typeof(FSharpGUI));
		...
		private void tbFind_KeyPress(object sender , KeyPressEventArgs e)
		{
			if(e.KeyChar == '\r' && CurrentTB != null)
			{
				...
				var ranges = r.GetRanges(pattern , RegexOptions.IgnoreCase);
				foreach(var found in ranges)
				{
					found.Inverse();
		...
		private void enableCodeMapToolStripMenuItem_Click(object sender , EventArgs e)
		{
			var ts = sender as ToolStripMenuItem;
			if(ts == null)
				return;

			docMap.Visible = ts.Checked ? true : false;
		}

		

		readonly List<FSharpInterctive> fsiList = new List<FSharpInterctive>();
		...
		private void Fsi_FormClosing(object sender , FormClosingEventArgs e)
		{
		...
		private void PowerfulCSharpEditor_KeyDown(object sender , KeyEventArgs e)
		{
