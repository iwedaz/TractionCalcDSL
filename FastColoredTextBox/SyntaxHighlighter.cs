...
namespace FastColoredTextBoxNS
{
	public class SyntaxHighlighter : IDisposable
	{
		...
		public readonly Style BoldStyle = new TextStyle(Brushes.SkyBlue, null, FontStyle.Bold);
		...
		protected Regex FSharpAttributeRegex,
					  FSharpClassNameRegex;

		protected Regex FSharpCommentRegex1,
					  FSharpCommentRegex2,
					  FSharpCommentRegex3;

		protected Regex FSharpKeywordRegex;
		protected Regex FSharpNumberRegex;
		protected Regex FSharpStringRegex;
		...
		protected void InitFShaprRegex()
		{
			//FSharpStringRegex = new Regex( @"""""|@""""|''|@"".*?""|(?<!@)(?<range>"".*?[^\\]"")|'.*?[^\\]'", RegexCompiledOption);
		...
		public virtual void FSharpSyntaxHighlight(Range range)
		{
			range.tb.CommentPrefix = "//";
			range.tb.LeftBracket = '(';
			range.tb.RightBracket = ')';
			...
			range.ClearStyle(StringStyle , CommentStyle , NumberStyle , AttributeStyle , ClassNameStyle , KeywordStyle);
			//
			if(FSharpStringRegex == null)
				InitFShaprRegex();
			//string highlighting
			range.SetStyle(StringStyle , FSharpStringRegex);
			//comment highlighting
			range.SetStyle(CommentStyle , FSharpCommentRegex1);
			range.SetStyle(CommentStyle , FSharpCommentRegex2);
			range.SetStyle(CommentStyle , FSharpCommentRegex3);
			//number highlighting
			range.SetStyle(NumberStyle , FSharpNumberRegex);
			//attribute highlighting
			range.SetStyle(AttributeStyle , FSharpAttributeRegex);
			//class name highlighting
			range.SetStyle(ClassNameStyle , FSharpClassNameRegex);
			//keyword highlighting
			range.SetStyle(KeywordStyle , FSharpKeywordRegex);
